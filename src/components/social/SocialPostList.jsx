...
export default function SocialPostList({ posts, allChannels, onPostSelect, onPostDelete }) {
    ...
    return (
        <Tabs defaultValue={allChannels[0]} className="w-full">
            <TabsList className="grid w-full h-auto p-2 bg-white/60 backdrop-blur-sm rounded-xl border border-slate-200 shadow-lg" style={{ gridTemplateColumns: `repeat(${Math.min(allChannels.length, 7)}, 1fr)` }}>
                {allChannels.map(channel => (
                    <TabsTrigger key={channel} value={channel} className="flex items-center gap-2 py-3 px-4 data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-[var(--primary)] transition-all duration-300 rounded-lg font-sans">
                    ...
                    {(groupedPosts[channel] && groupedPosts[channel].length > 0) ? (
                        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                            {groupedPosts[channel].map(post => (
                                <Card 
                                    key={post.id} 
                                    className="group relative bg-white border-0 shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer overflow-hidden"
                                ...
                                >
                                    ...
                                    <CardContent className="p-4 pt-6 relative">
                                        ...
                                        <div className="flex items-start justify-between mb-3">
                                            ...
                                            <Badge variant="outline" className={`text-xs font-sans ${post.status === 'Suggested' ? 'bg-yellow-100 text-yellow-800 border-yellow-300' : post.status === 'Scheduled' ? 'bg-purple-100 text-purple-800 border-purple-300' : 'bg-green-100 text-green-800 border-green-300'}`}>
                                                {post.status}
                                            </Badge>
                                        </div>
                                        
                                        <div className="mb-3 p-2 bg-slate-50/50 rounded-lg border-l-2 border-[var(--primary)]">
                                        ...
                                        <div className="flex items-center justify-between text-xs text-slate-500 font-sans pt-2 border-t border-slate-100">
                                            ...
                                            <span className="text-slate-400 group-hover:text-[var(--primary)] transition-colors">Click to view</span>
                                        </div>
                                    </CardContent>
                        ...
                        <div className="text-center py-16 text-slate-500 border-2 border-dashed border-slate-200 rounded-2xl bg-white">
                            ...
                            <div className="flex justify-center">
                              <div className="w-16 h-0.5 bg-[var(--primary)] rounded-full"></div>
                            </div>
                        </div>
                    )}
                </TabsContent>
