...
import { X, Save, Edit, Calendar as CalendarIcon, Bot, User, Loader2, Globe } from "lucide-react";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
import { Calendar } from "@/components/ui/calendar";
import { Badge } from "@/components/ui/badge";
import { format } from 'date-fns';
import { InvokeLLM } from '@/integrations/Core';
import { publishToWordPress } from '@/functions/publishToWordPress';
...
export default function PostEditorModal({ post, onSave, onClose }) {
  ...
  const handlePublishToWordPress = async () => {
    ...
    try {
      const response = await publishToWordPress({ blog_post_id: post.id });
      
      if (response.data.success) {
        toast.success("Published to WordPress!", {
          description: `Your blog post is now live at ${response.data.wordpress_url}`,
          duration: 6000,
        });
        ...
        toast.error("Publishing Failed", {
          description: response.data.error || "Could not publish to WordPress. Please try again.",
          duration: 6000,
        });
      }
    } catch (error) {
      ...
      toast.error("Publishing Failed", {
        description: "An unexpected error occurred. Please check your WordPress credentials and try again.",
        duration: 6000,
      });
    } finally {
      ...
      <Card className="w-full max-w-5xl bg-white shadow-2xl flex flex-col max-h-[95vh]">
                  ...
                  <div className="bg-white rounded-lg border relative">
                    ...
                    {isConverting ? (
                      ...
                      <>
                        ...
                        .modal-editor .ql-editor a {
                            color: #2563eb;
                            text-decoration: underline;
                            cursor: pointer;
                        }
                        .modal-editor .ql-editor a:hover {
                            color: #1d4ed8;
                        }
                        `}
                        </style>
                      ...
                      <div className="flex items-start gap-2 mb-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-green-700 mt-1">
                            This post is approved and ready to be published to your WordPress website.
                          </p>
                        </div>
                      </div>
                      <Button 
        ...
        <div className="flex gap-3 p-6 border-t border-slate-100 flex-shrink-0">
          ...
          <Button 
            onClick={handleSave} 
            disabled={isSaving}
            className="flex-1 h-11 bg-slate-800 text-white hover:bg-slate-900"
          >
            {isSaving ? (
              <div className="flex items-center gap-2">
  ...
  function getStatusColor(status) {
    switch (status) {
      ...
      case 'Scheduled': return 'bg-purple-100 text-purple-800 border border-purple-200';
