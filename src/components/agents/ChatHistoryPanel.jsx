  ...
  return (
    <div className="flex flex-col h-full bg-white/50 backdrop-blur-sm border rounded-xl shadow-lg">
      <div className="p-4 border-b space-y-4">
        ...
        <Button onClick={onNewConversation} className="w-full pcc-bg-blue text-white shadow-lg hover:opacity-90">
            ...
            {conversations.map((convo) => (
              <div key={convo.id} className="relative group">
                <button
                  ...
                  className={cn(
                    'w-full text-left pl-3 pr-12 py-2 rounded-md transition-all duration-200 flex items-center gap-3 font-sans',
                    selectedConversationId === convo.id
                      ? 'pcc-bg-coral bg-opacity-20 pcc-text-coral font-semibold'
                ...
                <div className="absolute right-1 top-1/2 -translate-y-1/2 flex items-center">
                   <Button
                      ...
                      variant="ghost"
                      className={cn(
                        "h-7 w-7 transition-opacity text-slate-500 hover:text-red-600",
